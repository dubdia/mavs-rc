...
const devTools = MAIN_WINDOW_WEBPACK_ENTRY != null || appConfigManager.config.devTools; // only enable when debugging or when configured
log.info(
...
app.on("ready", () => {
  // configure security restrictions
  {
    ...
    if (MAIN_WINDOW_WEBPACK_ENTRY) {
      allowedUrls.push(MAIN_WINDOW_WEBPACK_ENTRY, MAIN_WINDOW_WEBPACK_ENTRY.replace("http://", "ws://"));
      allowedUrls.push(
        MAIN_WINDOW_WEBPACK_ENTRY,
        MAIN_WINDOW_WEBPACK_ENTRY.replace("http://", "ws://").replace("main_window", "ws")
      );
    }
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      preload: MAIN_WINDOW_PRELOAD_WEBPACK_ENTRY, //currentPath().join(__dirname, "preload.js"),
  ...
  log.info(`Load from ${MAIN_WINDOW_WEBPACK_ENTRY}`);
