...
import { FaArchive, FaArrowRight, FaBars, FaDownload, FaFile, FaFolder, FaFolderPlus, FaInfo, FaSearch, FaTerminal, FaTrash, FaUpload, FaUserLock } from "react-icons/fa";
    ...
    <>
      <div className="w-full">
        ...
        >
          ...
          <TableBody items={explorer.filtered} isLoading={explorer.loading} loadingContent={<Spinner color="white" />}>
            {(item) => (
              <TableRow key={item.name}>
                ...
                {/* Icon & Name */}
                <TableCell className="font-mono font-bold text-white cursor-pointer" onClick={() => navigateToAsync(item)}>
                  <p
                    title={item.fullName}
                    key={item.fullName}
                    className={clsx(
                    ...
                    <Dropdown backdrop="blur">
                      ...
                      <DropdownMenu aria-label="Actions">
                        <DropdownSection title={item.name}>
                          {[
                            <DropdownItem key="navigateTo" onClick={async () => await navigateToAsync(item)} startContent={<FaArrowRight/>}>
                              {item.isRegularFile ? "Edit" : "Open"}
                            </DropdownItem>,
                            <DropdownItem key="delete" onClick={() => files.removeFile(item.fullName!)} startContent={<FaTrash/>}>
                              Delete
                            </DropdownItem>,
                            <DropdownItem key="rename" onClick={() => files.renameFile(item.fullName!)} startContent={<Fa0/>}>
                              Rename
                            </DropdownItem>,
                            <DropdownItem key="permissions" onClick={() => setShowPermissionsOf(item)} startContent={<FaUserLock/>}>
                              Permissions
                            </DropdownItem>,
                            <DropdownItem key="owner" onClick={() => setShowOwnerOf(item)} startContent={<FaUserLock/>}>
                              Owner User/Group
                            </DropdownItem>,
                            <DropdownItem key="info" onClick={() => files.showInfo(item.fullName!)} startContent={<FaInfo/>}>
                              Info
                            </DropdownItem>,
                            <DropdownItem key="download" onClick={() => files.download(item.fullName!)} startContent={<FaDownload/>}>
                              Download
                            </DropdownItem>,
                            ...(files.isArchiveFile(item.fullName)
                              ? [
                                  <DropdownItem key="unzip" onClick={() => files.unpackFile(item.fullName!)} startContent={<FaArchive/>}>
                                    Extract
                                  </DropdownItem>,
                                ]
                              : []),
