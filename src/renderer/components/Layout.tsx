...
export const Layout: React.FC<LayoutProps> = ({ name, header, body }) => {
  return (
    <div className="absolute top-0 left-0 bottom-0 flex flex-col w-full max-w-5xl">
      <div className="absolute top-0 left-0 right-0 flex flex-col justify-end backdrop-blur-lg backdrop-saturate-150 z-40 h-24 min-h-24 max-h-24 pr-8">
      ...
      <div className="absolute top-24 left-0 right-0 bottom-0 pr-8 pt-4 space-y-4">{body}</div>
    </div>
  );
};
